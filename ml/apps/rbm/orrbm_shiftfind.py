...
direct_acc = ml.rbm.accuracy.calc_separation_accuracy("direct",
                                                   ref_predict, None,
                                                   X, XZ, ref_XZ,
                                                   Y, YZ, ref_YZ,
                                                   O, OX, OY)       
...
if load_H:
    ...
    H = -999999 * np.ones((cfg.n_samples, 2*rbm.orrbm.base_y, ml.rbm.orrbm.width))
    for x_shift, y_shift in itertools.product(cfg.test_x_shifts, cfg.test_y_shifts):
        ...
        Hshift = ml.common.util.map(O, map_batch,
...
for x_shift, y_shift in itertools.product(cfg.test_x_shifts, cfg.test_y_shifts):
    ...
    sep_XY = ml.common.util.map(Oshift, map_batch,
...
orrbm_acc = ml.rbm.accuracy.calc_separation_accuracy("maxH",
                                                  ref_predict, myrbm,
                                                  X, XZ, ref_XZ,
                                                  Y, YZ, ref_YZ,
                                                  O,
