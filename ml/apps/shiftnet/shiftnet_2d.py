...
from ml.datasets.shift_gpu import generate_data_2d
import ml.nn.shift2d
...
plt.clf()
plot_all_weights(ps)
plt.suptitle("final iter=%d" % iteration)
plt.draw()
if ml.common.plot.headless:
    plt.savefig(plot_dir + "/weights_final.pdf")
...
threshold = None
print "input:   "
print format_sample(sim_inputs[:, 0], cfg.width, cfg.height, threshold=threshold)
print "shift:   "
print sim_shifts[:, 0]
print "targets: "
print format_sample(sim_targets[:, 0], cfg.width, cfg.height, threshold=threshold)
print "results: "
print format_sample(sim_results[:, 0], cfg.width, cfg.height, threshold=threshold)

# test on global test set
ts_filename = os.path.abspath(__file__) + "testsets/%d_%d.npz" % (cfg.width, cfg.height)
