...
from ml.common import util
from ml.common import dlutil
from ml.common.util import myrand
...
def load_parameters(rbm, epoch_or_filename):
    ...
    ml.rbm.weights = gp.as_garray(state['weights'])
    ml.rbm.bias_vis = gp.as_garray(state['bias_vis'])
    ml.rbm.bias_hid = gp.as_garray(state['bias_hid'])
