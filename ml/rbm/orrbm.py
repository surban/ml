...
from ml.common.util import flatten_samples, unflatten_samples_like
...
def or_infer(rbm, vis, iters, k, beta=1):
    ...
    for i in range(iters):
        xs, _ = ml.rbm.gibbs_sample(xi, k, vis_force=xf, beta=beta)
        yi, yf = or_rest(vis, xs)
        ys, _ = ml.rbm.gibbs_sample(yi, k, vis_force=yf, beta=beta)
